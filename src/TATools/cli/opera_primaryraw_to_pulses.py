...
def _seek_until_separator(file_handle: io.BufferedReader, separator: bytes = bytes([0xff, 0xff, 0x00, 0x00, 0xff, 0xff])):
    n = len(separator)
...
def _read_counts(file_handle: io.BufferedReader) -> TeensyCounts:
    ...
    for i in range(num_pulse):
        ...
        pulse = Pulse(pulse_data[0], pulse_data[1], pulse_data[2], pulse_data[3], list(pulse_data[2:]))
